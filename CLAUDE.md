...
- Wire DTOs (`LoginRequest`, `LoginResponse`, `LoginOrgChoiceResponse`, `SelectOrgRequest`, `OrganizationSummaryDto`) live in `net.calvuz.qstore.shared.dto`, a Kotlin Multiplatform module in the sibling repo `../quickstore-shared` (JVM-only target for now), included via `includeBuild` in `settings.gradle.kts` and consumed by both this app and `quickstore-server` — no more hand-duplicated DTO files kept in sync manually. See `quickstore-shared`'s own repo for the module itself; this is prep work for a possible future web client (Next.js/TypeScript, not KMP-on-web) that would talk to `quickstore-server` directly.
...
- **Push**: queries each DAO's `getUpdatedSince(cursor)` (`getCreatedSince` for `movements`, append-only) across all 6 synced entities (`article_categories`, `locations`, `articles`, `article_location_thresholds`, `movements`, `article_images`), maps to the DTOs shared with `quickstore-server` via `quickstore-shared` (see "Auth" section above). `movements.createdBy` falls back to the current session's `userId` if a row predates any login.
...
- **Article detail shows location-scoped stock, not just the total (fixed 2026-07-26).** Real bug: `ArticleDetailScreen`'s inventory card always showed the TOTAL quantity summed across every location, even when the user had reached that article by filtering the article list to one specific warehouse — e.g. filtering to "Sede" (showing "2"), tapping the article, and seeing "9" (2+7 across two locations) on the detail screen, with no obvious way on a small screen to tell which number belonged to which location. Fixed by having `ArticleDetailViewModel` also observe the same persistent "active location" filter the article list already uses (`GetActiveLocationUseCase`/`ActiveLocationRepository`, a DataStore-backed global setting, not scoped to a single screen) via a new `ObserveLocationQuantityUseCase`/`InventoryRepository.observeQuantityAt()`. No navigation changes needed — the active location is already global app state, so the detail screen just reads the same source of truth the list used to decide what number to show when it was tapped. When a location is active, the card's title becomes "Giacenza a {nome}" and its headline number is the quantity in that location; the total across all locations is still shown, just demoted to a small secondary line ("Totale su tutti i magazzini: X") instead of disappearing — a deliberate choice given the user's small-screen complaint (no room for extra cards/rows, just re-prioritize the existing one). When no location filter is active ("Tutti i magazzini"), behavior is unchanged from before (title "Giacenza Attuale", total as the headline number). Verified live on the emulator: filtered to "Magazzino principale", opened the article split 7/2 across two locations, card read "Giacenza a Magazzino principale — 7.0 PZ" / "Totale su tutti i magazzini: 9.0 PZ".
...
- **Paginazione lista articoli**: oggi `ArticleDao.observeAll()` carica sempre l'intera tabella in un `Flow<List<Article>>`; filtri (ricerca, categoria) e ordinamento sono applicati in Kotlin in `ArticleListViewModel.applyFiltersAndSort`, non in SQL. `LazyColumn` virtualizza già il rendering, quindi non è un problema per centinaia di articoli. Da rivalutare solo se si arriva nell'ordine delle migliaia: soluzione idiomatica sarebbe `Paging 3` (`PagingSource` via Room + `LazyPagingItems`), che richiede però di spostare prima ricerca/filtro/ordinamento da Kotlin a query SQL lungo tutta la catena DAO → Repository → UseCase → ViewModel.
